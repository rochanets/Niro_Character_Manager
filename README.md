...
| **Banners** | Grade de banners por versão (colunas 1.x–8.x com nome) e subversão (linhas x.0–x.8). Tipos: Unitário (1×5⭐ + 3×4⭐), Duplo (2×5⭐ + 3×4⭐) e Especial (até 12×5⭐ + 4⭐ ilimitado), que fica recolhido e expande ao clicar no cabeçalho. Personagens são adicionados via modal com filtros. Quem aparece em um banner Especial fica bloqueado nas demais seleções do mesmo ciclo (versão x.*); aparições em banners comuns não bloqueiam. |
